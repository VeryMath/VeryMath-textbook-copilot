...
- `range`：只处理 `pageRange.start` 到 `pageRange.end` 的 PDF 页序，包含首尾。即使跨章也不扩大范围，按选定内容组织课件。
- `section`：当前二级目录节及其下属内容，不扩大到整章。
- `chapter`：当前一级目录章及其下属小节。根据目录找到下一项同级或更高层级标题，结合正文确定结束位置；同页的小节边界通过正文标题判断。
