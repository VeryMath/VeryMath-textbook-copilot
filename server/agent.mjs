...
可用的本机 Node.js：${process.env.ELECTRON_RUN_AS_NODE === '1' ? (process.platform === 'win32' ? `在 PowerShell 中使用 $env:ELECTRON_RUN_AS_NODE='1'; & '${process.execPath.replaceAll("'", "''")}'` : `env ELECTRON_RUN_AS_NODE=1 "${process.execPath}"`) : process.execPath}。教材读取工具：${fileURLToPath(new URL('../skills/textbook-parse/scripts/read-pages.mjs', import.meta.url))}，接受 --pdf、--start、--end、--out；纯文字读取用 --images none，需要原页校对用 --images pages，需要独立图片用 --images all；--out 使用当前课程 ${resolve(context.outputsDir, '.build')} 下的子目录。
